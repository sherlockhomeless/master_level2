...
void reschedule(Plan* p, short);

// --- debug ---
void get_all_ids_from_plan(long[400], Plan*);
short same_ids(long[400], long[400]);
short order_is_kept(Plan*);

void handle_no_preemption_slot_found(Plan *p);

void signal_t2(Plan* p){
    change_plan_state(p, SIGNALED);
    printf("[SIGNAL_T2]%ld: signaled prediction failure for process %ld", p->tick_counter, p->cur_process->process_id);
    p->stress = STRESS_PER_SIGNAL;
    reschedule(p, STRETCH_SIGNAL);
}

void signal_tm2(Plan* p){
    change_plan_state(p, SIGNALED);
    printf("[SIGNAL_T-2]%ld: signaled prediction failure for task %ld\n",p->tick_counter, p->cur_task->task_id);
    p->stress = STRESS_PER_SIGNAL;
...
void preempt_cur_task(Plan* p){
   ...
   int stack_size;
   long insertion_slot;
   Task preempted_tasks[T2_MAX_PREEMPTIONS+1] = {0};
    ...
    if(LOG)
        printf("[PREEMPT_CUR_TASK]%ld: %d Tasks where preempted and moved into slot %ld before Task %ld\n", p->tick_counter, stack_size,
               insertion_slot , p->tasks[insertion_slot+1].task_id);

    show_tasks(p);
}

void handle_no_preemption_slot_found(Plan *p) {//fixme: how to deal with this issue? State: Just sign Task finished
    ...
    printf("[EXITING]%ld: no preemption slot found", p->tick_counter);
    exit(0);
}
...
long find_slot_to_move_to(long target_pid, Plan* p){
    ...
    while (pid != -2){
        ...
        if (pid == target_pid){
            printf("[FIND_SLOT] found slot %ld for (%ld, %ld)\n", counter - 1, p->cur_task->task_id, p->cur_task->process_id);
            assert(counter >= 1 );
            return counter - 1;
        }
        next_slot++;
...
void move_preempted_tasks(long insertion_slot, int stack_size, Task *preempted_tasks, Plan *p) {
    ...
    Task task_before = *cur_task;

    for (int i = 0; i < stack_size; i++){
        *(cur_task + i) = preempted_tasks[i];

        printf("[MOVE_PREEMPTED_TASK]%ld: Moved (%ld,%ld) in place of (%ld,%ld)\n", p->tick_counter,
...
void reschedule(Plan* p, short signal){
    ...
    printf("[RESCHEDULE]%ld: Received %d signal and stretched/shrunk %ld tasks\n", p->tick_counter, signal, task_counter);
    }
